...
typedef struct _map_t _map_t;

// $ => enum; 
// ? => bool; 
// ! => mask

static JanetKV* _options = NULL;

static _map_t _curl_opts_map[] = {  
...
void opts_gen_dict(void) {
  int32_t size = sizeof(_curl_opts_map) / sizeof(_map_t);

  _options = janet_struct_begin(1);

  for (int32_t idx = 1; idx < size; idx++){    
    const Janet key = janet_ckeywordv(_curl_opts_map[idx].key);
    JanetKV* optyp = janet_struct_begin(1);

    switch(_curl_opts_map[idx].type) {
      case JANET_NUMBER:
        janet_struct_put(
          optyp,
          janet_wrap_number(JANET_NUMBER),
          janet_wrap_number(_curl_opts_map[idx].val));
        break;
      case JANET_BOOLEAN:
        janet_struct_put(
          optyp,
          janet_wrap_number(JANET_BOOLEAN),
          janet_wrap_number(_curl_opts_map[idx].val));
        break;
      case JANET_STRING:
        janet_struct_put(
          optyp,
          janet_wrap_number(JANET_STRING),
          janet_wrap_number(_curl_opts_map[idx].val));
        break;
    ...
    janet_struct_put(_options, key, janet_wrap_struct(optyp));
    janet_struct_end(optyp);
  }
  janet_struct_end(_options);
}

static const JanetKV* _opts_get(Janet* key) {
  Janet value = janet_struct_get(_options, *key);
  return janet_unwrap_struct(value);
}

int opts_set(CURL* curl, Janet* key, Janet* val) {
  const JanetKV* optkv = _opts_get(key);

  const JanetType type = janet_unwrap_number(optkv->key);
  const int32_t opt = janet_unwrap_integer(optkv->value);
